...
class FMeasure:
    ...
    def _compute_precision_recall(self, binary_pred_mask: torch.Tensor, gt_mask: torch.Tensor) -> torch.Tensor:
        """
    ...
    def _compute_f_measure(
        ...
        f_measure = ((1 + (self.beta_square ** 2)) * prec * recall) / ((self.beta_square ** 2) * prec + recall + self.eps)
        return f_measure.cpu()

    def _compute_f_max(self, pred_mask: torch.Tensor, gt_mask: torch.Tensor) -> torch.Tensor:
        ...
        thresholds = torch.arange(0, 1, 1 / self.n_bins).view(self.n_bins, 1, 1).to(pred_masks.device)

        # f_measures: self.n_bins
        f_measures = self._compute_f_measure(pred_masks, gt_masks, thresholds)
        return torch.max(f_measures).cpu()
    ...
    def _compute_f_mean(
        ...
        prec, recall = self._compute_precision_recall(binary_pred_mask, gt_mask)
        f_mean = ((1 + (self.beta_square ** 2)) * prec * recall) / ((self.beta_square ** 2) * prec + recall + self.eps)
        return f_mean.cpu()
    ...
    def __call__(self, pred_mask: torch.Tensor, gt_mask: torch.Tensor) -> dict:
        ...
        """
        outputs: dict = dict()
        for k in ("f_measure", "f_max", "f_mean"):
